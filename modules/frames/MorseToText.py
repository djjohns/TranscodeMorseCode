from customtkinter import CTk, CTkFrame, CTkLabel, CTkButton, filedialog
from dataclasses import dataclass
from modules.TranscodeMorseCode import decrypt
import datetime


def get_timestamp():
    """
    ### Gets current string formatted timestamp in YYYY-MM-DD_HH-MM-SS format.
    """
    # Get the current timestamp as a datetime object
    current_timestamp = datetime.datetime.now()
    # Format the timestamp as a string with a specific format
    TS = current_timestamp.strftime("%Y-%m-%d_%H-%M-%S")
    return TS


def write_txt_file(filename, file_content):
    text_file = open(filename, "w")
    text_file.write(file_content)
    text_file.close()


def morse_to_text():
    TS = get_timestamp()
    filename = f"Incoming_CW_{TS}.txt"
    default_file_loc = f"D:/dev/ham_radio_apps/TranscodeMorseCode/data/rx/"

    def read_txt_file():
        try:
            with filedialog.askopenfile(
                initialdir=default_file_loc,
                initialfile="MorseCodeFile.txt",
                title="Select Morse Code File",
            ) as file:
                file_contents = file.read()
                print("File contents:")
                print(file_contents)
            return file_contents
        except FileNotFoundError:
            print(f"File not found.")
        except Exception as e:
            print(f"An error occurred: {e}")

    message = read_txt_file()

    # If we get the user's input.
    if message:
        # Transcode the Morse Code into a string.
        result = decrypt(message.upper())
        # Write the transcoded results to a txt file.
        write_txt_file(f"{default_file_loc}{filename}", result)
        # TODO: Setup logging and pass this to log.
        print(f"\n")
        print(f"{filename:-^80}")
        print(f"Morse Code:{message}")
        print(f"Message:{result}")
        print(f"\n")


@dataclass
class MorseToText:
    """
    Example Widget.
    """

    master_app: CTk

    def frame(self):
        """
        Sets UI details for the Widget frame.
        """
        frame = CTkFrame(master=self.master_app, fg_color="#606190")
        frame.grid(row=0, column=1, rowspan=2, padx=50, pady=50)

        CTkLabel(
            master=frame,
            text=f"Morse Code to text",
            font=("Arial Bold", 20),
            justify="center",
        ).pack(expand=True, pady=(30, 15))

        CTkButton(master=frame, text=f"Click Me", command=morse_to_text).pack(
            expand=True, fill="both", pady=(30, 15), padx=30
        )
